...
from customize.util import dict_to_json, get_current_gpu_usage, dict_to_tb
...
class Logger:
    def __init__(self, name='train', task_type=None):
        ...
        self._epoch_total = cfg.trainer.epochs
    ...
    def basic(self):
        stats = {
            'loss': round(self._loss / self._size_current, cfg.rounding),
            # 'lr': round(self._lr, cfg.rounding),
            # 'params': self._params,
            'time_iter': round(self.time_iter(), cfg.rounding),
        }
    ...
    def _get_pred_int(self, pred_score):
        if len(pred_score.shape) == 1 or pred_score.shape[1] == 1:
            return (pred_score > cfg.model.threshold).long()
        else:
    ...
    def classification_binary(self):
        ...
        try:
            ...
            r_a_score = 0.0
        return {
            'accuracy': round(accuracy_score(true, pred_int), cfg.rounding),
            'precision': round(precision_score(true, pred_int), cfg.rounding),
            'recall': round(recall_score(true, pred_int), cfg.rounding),
            'f1': round(f1_score(true, pred_int), cfg.rounding),
            'auc': round(r_a_score, cfg.rounding),
        }
    ...
    def classification_multi(self):
        ...
        true, pred_score = torch.cat(self._true), torch.cat(self._pred)
        pred_int = self._get_pred_int(pred_score)
        return {'accuracy': round(accuracy_score(true, pred_int), cfg.rounding)}
    ...
    def regression(self):
        ...
        return {
            'mae':
            float(round(mean_absolute_error(true, pred), cfg.rounding)),
            'mse':
            float(round(mean_squared_error(true, pred), cfg.rounding)),
            'rmse':
            float(round(math.sqrt(mean_squared_error(true, pred)), cfg.rounding))
        }
    ...
    def update_stats(self, true, pred, loss, time_used, **kwargs):
    ...
    def write_epoch(self, cur_epoch):
        ...
        for custom_metric in cfg.custom_metrics:
            func = registry['metric'][custom_metric]
            if not func:
                raise ValueError(
        ...
        eta_stats = {'eta': round(self.eta(cur_epoch), cfg.rounding)}
...
def infer_task():
    num_label = cfg.model.params.out_channels
...
def create_logger():
    ...
    for i, dataset in enumerate(range(cfg.trainer.num_splits)):
